...
//            AbstractDetailCRUDController, NestedFormController, Selection, ModelController, 
//            FormController

public class RemoteDetailCRUDController extends AbstractDetailCRUDController
    implements Serializable
{

    public RemoteDetailCRUDController(String s, Class class1, String s1, String s2, FormController formcontroller)
    {
        this(s, class1, s1, s2, formcontroller, true);
    }

    public RemoteDetailCRUDController(String s, Class class1, String s1, String s2, FormController formcontroller, boolean flag)
    {
        ...
        try
        {
            constructor = class1.getConstructor(new Class[] {
                it.cnr.jada.util.action.CRUDController.class
            });
        }
    ...
    {
        return (CRUDDetailComponentSession)createComponentSession(componentSessionName, it.cnr.jada.ejb.CRUDDetailComponentSession.class);
    }

    public Object createComponentSession(String s, Class class1) throws BusinessProcessException{
        try{
    ...
    protected RemoteIterator createRemoteIterator(ActionContext actioncontext)
    {
        try
        {
            return EJBCommonServices.openRemoteIterator(actioncontext, createComponentSession().cerca(actioncontext.getUserContext(), filter, modelClass, getParentModel(), attributeName));
        }
    ...
    private List fetchPage(int i)
    {
        try
        {
            ...
            while(j++ < k) 
                arraylist.add(initializeForEdit((OggettoBulk)remoteIterator.nextElement()));
            return arraylist;
        }
        catch(RemoteException remoteexception)
        {
    ...
    protected OggettoBulk getDetail(int i)
    {
        try
        {
            ...
            } else
            {
                ...
                return initializeForEdit((OggettoBulk)remoteIterator.nextElement());
            }
        }
        catch(RemoteException remoteexception)
        {
        ...
        throws ValidationException, BusinessProcessException
    {
        ...
        BitSet bitset = super.selection.getSelection(getCurrentPage() * getPageSize(), getPageSize());
        if(bitset.length() > 0)
        {
            int i = 0;
        ...
        if(super.selection.getFocus() >= 0)
        {
            ...
            removeDetails(actioncontext, new OggettoBulk[] {
                oggettobulk
            });
        }
        ...
        throws ValidationException, BusinessProcessException
    {
        try
        {
            createComponentSession().eliminaConBulk(actioncontext.getUserContext(), getParentModel(), attributeName);
            reset(actioncontext);
        }
        catch(Throwable throwable)
        {
    ...
    protected void removeDetails(ActionContext actioncontext, OggettoBulk aoggettobulk[])
        throws BusinessProcessException
    {
        try
        {
            createComponentSession().eliminaConBulk(actioncontext.getUserContext(), aoggettobulk, getParentModel(), attributeName);
        }
        catch(RemoteException remoteexception)
        {
        ...
        throws BusinessProcessException
    {
        try
        {
            createComponentSession().modificaConBulk(actioncontext.getUserContext(), getParentModel(), oggettobulk, attributeName);
    ...
    protected void setModel(ActionContext actioncontext, OggettoBulk oggettobulk)
    {
        ...
        {
            if(oggettobulk != null)
                oggettobulk = createComponentSession().inizializzaBulkPerModifica(actioncontext.getUserContext(), oggettobulk, getParentModel(), attributeName);
        }
        catch(Throwable throwable)
        {
    ...
    public void updateDetail(ActionContext actioncontext, OggettoBulk oggettobulk)
        throws BusinessProcessException
    {
        try
        {
            createComponentSession().modificaConBulk(actioncontext.getUserContext(), getParentModel(), getModel(), attributeName);
        }
        catch(Throwable throwable)
        {
            throw new BusinessProcessException(throwable);
    ...
    {
        writeHTMLPagedTable(pagecontext, s, flag, flag1, flag2, s1, s2, flag3, tablecustomizer, cachedDetailsPage, calcPageCount(remoteIterator.countElements()));
    }
