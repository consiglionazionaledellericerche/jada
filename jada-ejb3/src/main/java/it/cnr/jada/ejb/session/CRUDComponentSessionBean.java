 ...
 */
@Stateless
public class CRUDComponentSessionBean<T extends OggettoBulk> extends AbstractComponentSessionBean<T>  implements CRUDComponentSessionLocal<T> {	
    /**
     * Default constructor. 
     */
	...
	public T findByPrimaryKey(UserContext userContext, T oggettoBulk) throws ComponentException{
    	return getHomeClass(oggettoBulk).findByPrimaryKey(userContext, oggettoBulk);
    }
    ...
    /**
     * Implementazione fisica del metodo cerca(UserContext,CompoundFindClause,T). 
     * Costruisce l'istruzione SQL corrispondente ad una ricerca con le clausole specificate. 
     * L'implementazione standard invoca il metodo findByCriteria sull'Home dell'T specificato 
     * come prototipo.
     */
    protected  Criteria select(UserContext userContext, Criterion criterion, T oggettoBulk) throws ComponentException{
    	return getHomeClass(oggettoBulk).selectByCriterion(userContext, criterion);
    }

    protected  Criteria select(UserContext userContext, Criterion criterion, Class<T> bulkClass) throws ComponentException{
    	return getHomeClass(bulkClass).selectByCriterion(userContext, criterion);
    }
    ...
    /**
     * Esegue una operazione di creazione di un T. 
     * Pre-post-conditions: 
     * 	Nome: Non passa validazione applicativa 
     * 	Pre: l'T non passa i criteri di validità applicativi 
     ...
     * 	Pre: l'T contiene qualche attributo nullo in corrispondenza di campi NOT_NULLABLE o 
     ...
     * 	Pre: Esiste già un T persistente che possiede la stessa chiave primaria di quello specificato. 
     ...
     * 	Nome: Vincolo di integrità referenziale Pre: l'T specificato non può essere creato perchè 
     ...
     * Post: l'T viene crato fisicamente nella base dati e viene chiusa la transazione
     */
     ...
     */
    protected  T eseguiCreaConBulk(UserContext usercontext, T oggettobulk) throws ComponentException, PersistencyException{
     ...
     */
    protected  void validaCreaConBulk(UserContext userContext, T oggettobulk) throws ComponentException{
        if(oggettobulk.isToBeCreated()){
            T oggettobulk1 = (T)getHomeClass(oggettobulk).findByPrimaryKey(userContext, oggettobulk);
     ...
     */
    protected  void validaCreaModificaConBulk(UserContext userContext, T oggettobulk) throws ComponentException{
        try{
    ...
    /**
     * Esegue una operazione di eliminazione di un T. 
     * Pre-post-conditions: 
     * Nome: Non passa validazione di business 
     * Pre: l'T non passa i criteri di validità di business per l'operazione di cancellazione 
     ...
     * Pre: l'T specificato non esiste. 
     ...
     * Pre: l'T specificato è bloccato da qualche altro utente. 
     * Post: Viene generata una CRUDException con la descrizione dell'errore 
     * Nome: Oggetto non cancellabile 
     * Pre: l'T non è cancellabile per motivi applicativi 
     * Post: Viene generata una CRUDNotDeletableException con la descrizione dell'errore 
     * Nome: Vincolo di integrità referenziale 
     * Pre: l'T specificato non può essere cancellato perchè viene violato+ qualche vincolo di integrità referenziale 
     * Post: Viene generata una CRUDReferentialIntegrityException con la descrizione dell'errore 
     * Nome: Tutti i controlli superati 
     * Pre: Tutti i controlli precedenti superati 
     * Post: l'T eliminato fisicamente dalla base di dati e viene chiusa la transazione
     */
     ...
     */
    protected  void validaEliminaConBulk(UserContext usercontext, T oggettobulk) throws ComponentException{
        try{
            validateBulkForPersistency(usercontext, oggettobulk);
        }catch(Throwable throwable){
            throw handleException(throwable);
     ...
     */
    protected  void eseguiEliminaConBulk(UserContext usercontext, T oggettobulk) throws ComponentException, PersistencyException{
        try{
    ...
    /**
     * Prepara un T per la presentazione all'utente per una possibile operazione di modifica. 
     * Pre-post-conditions: 
     * Nome: Oggetto non esistente 
     * Pre: L'T specificato non esiste. 
     * Post: Viene generata una CRUDException con la descrizione dell'errore. 
     * Nome: Tutti i controlli superati 
     * Pre: L'T specificato esiste. 
     * Post: Viene riletto l'T, inizializzato con tutti gli oggetti collegati e preparato 
	...
	public  T inizializzaBulkPerModifica(UserContext userContext, T oggettobulk) throws ComponentException {
        try{
            oggettobulk = getHomeClass(oggettobulk).findByPrimaryKey(userContext,oggettobulk);
            if(oggettobulk == null){
    ...
    /**
     * Prepara un T per la presentazione all'utente per una possibile operazione di ricerca libera. 
     * Nome: Tutti i controlli superati 
     * Pre: 
     * Post: l'T viene inizializzato con tutti gli oggetti collegati e preparato 
    ...
    /**
     * Esegue una operazione di modifica di un T. 
     * Pre-post-conditions: 
     * Nome: Non passa validazione applicativa 
     * Pre: l'T non passa i criteri di validità applicativi per l'operazione di modifica 
     * Post: Viene generata CRUDValidationException che descrive l'errore di validazione. 
     * Nome: Non passa validazione per violazione di vincoli della base di dati 
     * Pre: l'T contiene qualche attributo nullo in corrispondenza di campi NOT_NULLABLE 
     ...
     * Nome: Tutti i controlli superati 
     * Pre: Tutti i controlli precedenti superati 
     * Post: l'T viene modificato fisicamente nella base dati e viene chiusa la transazione
     */
     ...
     */
    protected  void validaModificaConBulk(UserContext usercontext, T oggettobulk) throws ComponentException{
        validaCreaModificaConBulk(usercontext, oggettobulk);
    }
     ...
     * Esegue una la parte di persistenza di creaConBulk.
     */
    protected  T eseguiModificaConBulk(UserContext userContext, T oggettobulk) throws ComponentException, PersistencyException{
        makeBulkPersistent(userContext, oggettobulk);
        return oggettobulk;
    }
